import { Transaction, DocketConfig } from "../../../../types";

const defaultConfig: DocketConfig = {
...
export function printDocket(tx: Transaction, docketConfig?: DocketConfig): boolean {
  const printWindow = window.open("", "_blank");
  if (!printWindow) return false;

  const config = docketConfig || defaultConfig;

  printWindow.document.write(`
    <html>
      <head>
        <title>Weighbridge Ticket - ${tx.ticketNo}</title>
        <style>
        ...
        <div class="page">
          <div>
            ...
            <table class="details-table">
              <tr>
                ...
                <td class="val-col">${tx.firstWeighTime}</td>
                <td class="label-col">DATE & TIME OUT:</td>
                <td class="val-col">${tx.secondWeighTime || "PENDING OUTBOUND"}</td>
              </tr>
              <tr>
                <td class="label-col">CUSTOMER / DEBTOR:</td>
                <td class="val-col" style="font-size: 11px; font-weight: 900;">${tx.customerName}</td>
                <td class="label-col">JOB / REFERENCE:</td>
                <td class="val-col">${tx.jobOrder || "N/A"}</td>
              </tr>
              <tr>
                <td class="label-col">CARRIER / TRANSPORT:</td>
                <td class="val-col">${tx.carrierName}</td>
                <td class="label-col">VEHICLE REG NO:</td>
                <td class="val-col" style="font-family: 'JetBrains Mono', monospace; font-weight: 900;">${tx.vehicleReg}</td>
              </tr>
              <tr>
                <td class="label-col">DRIVER NAME:</td>
                <td class="val-col">${tx.driverName}</td>
                <td class="label-col">DOCKET NUMBER:</td>
                <td class="val-col" style="font-family: 'JetBrains Mono', monospace; font-size: 12px; font-weight: 900; color: #1e293b;">${tx.ticketNo}</td>
              </tr>
              <tr>
                <td class="label-col">MATERIAL PRODUCT:</td>
                <td class="val-col" style="font-weight: 900; color: #1e293b;">${tx.productName}</td>
                <td class="label-col">LOT NUMBER:</td>
                <td class="val-col" style="font-family: 'JetBrains Mono', monospace; font-weight: 700;">${tx.lotNo || "N/A"}</td>
              </tr>
              <tr>
                <td class="label-col">INBOUND SCALE:</td>
                <td class="val-col" style="font-family: 'JetBrains Mono', monospace;">${tx.scaleIdInbound}</td>
                <td class="label-col">WEIGHBRIDGE SITE:</td>
                <td class="val-col">${config.weighbridgeLocation}</td>
              </tr>
            </table>

            <!-- WEIGHT BLOCK SECTION -->
            <div class="weights-box">
              <table class="weights-table">
                ...
                <tbody>
                  <tr>
                    <td>${tx.grossWeight.toFixed(2)}</td>
                    <td>${tx.tareWeight.toFixed(2)}</td>
                    <td style="font-size: 16px; font-weight: 900; color: #1e293b;">${tx.netWeight.toFixed(2)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div class="comments-box">
              ...
              <p style="margin: 0;">Own Driver. Wear appropriate high-vis PPE, safety glasses, and steel caps at all times. Follow strict 20 km/h quarry limits. Weighbridge certified under scale ID ${tx.scaleIdInbound}.</p>
            </div>

            <!-- PRICING CONDITIONAL BLOCK (DO NOT SHOW PRICING FOR ACCOUNT TRANSACTIONS) -->
            ${tx.type !== "Account" ? `
            <div class="pricing-section">
              <table class="pricing-table">
                <tr>
                  <td style="font-weight: 700; color: #475569;">BILLING CLASS / PAYMENT TYPE:</td>
                  <td style="text-align: right; font-weight: 800; color: #0f172a;">${tx.type} sale</td>
                </tr>
                <tr>
                  <td style="color: #475569;">Material Unit Base Rate:</td>
                  <td style="text-align: right; font-family: 'JetBrains Mono', monospace;">$${tx.basePrice.toFixed(2)} / t</td>
                </tr>
                <tr>
                  <td style="color: #475569;">Gross Billable Value (Ex. GST):</td>
                  <td style="text-align: right; font-family: 'JetBrains Mono', monospace;">$${tx.totalValue.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                </tr>
                <tr class="pricing-total">
                  <td style="font-weight: 900;">TOTAL DUE (EX. GST):</td>
                  <td style="text-align: right; font-family: 'JetBrains Mono', monospace; font-size: 14px; font-weight: 900; color: #2563eb;">$${tx.totalValue.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                </tr>
              </table>
            </div>
            ` : `
          ...
          <div>
            ...
            <div class="barcode-box">
              ||||| | ||||| | || | ||| |||| | ||||| | |||
              <div style="font-size: 7px; color: #94a3b8; margin-top: 2px;">TXN AUTH CODE: ${tx.transactionCode || tx.id}</div>
            </div>
            ...
            <table class="signatures-table">
              <tr>
                <td>
                  ...
                  <div>Operator Authorized Signature (ID: ${tx.operatorId})</div>
                </td>
                <td>
                  <div class="signature-line"></div>
